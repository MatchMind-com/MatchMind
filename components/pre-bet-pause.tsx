...
export default function PreBetPause({ bet, onClose, recentBets }: Props) {
  ...
  return (
    ...
    >
      <div
        className="bg-[#0E1628] border border-white/10 w-full max-w-md p-6 shadow-2xl"
      ...
      >
        ...
        {/* Bet details */}
        <div className="bg-white/[0.03] border border-white/[0.06] p-3 mb-4">
        ...
        {assessment && !loading && (
          <div className={`${meta.bg} border ${meta.border} p-4 mb-4`}>
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={proceed}
            className={`flex-1 ${assessment ? meta.cta : 'bg-emerald-600 hover:bg-emerald-500'} text-white font-bold py-2.5 text-sm transition-all`}
          >
            Place Bet
          </button>
        </div>
