...
interface Props {
  /**
   * Optional bankroll value from the parent BankrollTracker. If a goal
   * already exists this is ignored, but it pre-fills the "starting bankroll"
   * field when the user is setting their first goal so the two features
   * feel connected.
   */
  suggestedStartingBankroll?: number
}
...
export default function GoalTracker({ suggestedStartingBankroll }: Props) {
  ...
  const initialStart =
    suggestedStartingBankroll && suggestedStartingBankroll > 0
      ? String(suggestedStartingBankroll)
      : '100'

  // Form state
  const [startingBankroll, setStartingBankroll] = useState(initialStart)
  ...
  const trajectory = useMemo(() => {
    ...
    for (let i = 1; i <= buckets; i++) {
      ...
      const actual = isPast
        ...
        : null
      out.push({ label: `D+${dayOffset}`, targetVal, actual })
    }
    return out
  }, [goal])
  ...
  if (loading) {
    return (
      <div className="h-32 bg-white/[0.03] border border-white/[0.06] rounded-2xl animate-pulse" />
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h2 className="text-white text-xl lg:text-2xl font-black">Set a goal. Trust the math.</h2>
      ...
      {showForm && (
        <div className="bg-[#0E1628] border border-white/[0.08] rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full bg-white/[0.04] border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm focus:border-orange-500/50 focus:outline-none"
              />
            </div>
            <div>
              <label className="block text-white/50 text-xs font-bold mb-1.5 uppercase tracking-wide">
          ...
          <div className="flex gap-2 mt-5">
            ...
            <button
              onClick={save}
              disabled={saving}
              className="flex-1 bg-orange-500 hover:bg-orange-400 disabled:opacity-50 text-white font-black py-2.5 rounded-xl text-sm transition-all"
            >
      ...
      {goal && plan && !editing && (
        <>
          ...
          <div className="bg-[#0E1628] border border-white/[0.08] rounded-2xl p-5">
          ...
          <div className="bg-[#0E1628] border border-orange-500/20 rounded-2xl p-5">
          ...
          <div className="bg-[#0E1628] border border-white/[0.08] rounded-2xl p-5">
            ...
            <div className="flex items-center gap-4 mt-3 text-[11px]">
              ...
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 bg-emerald-500/40 border border-emerald-400 rounded-sm" />
          ...
          <div>
            <button
              ...
              className="bg-white/[0.05] hover:bg-white/[0.1] border border-white/10 text-white/70 font-bold px-4 py-2 rounded-xl text-sm transition-all"
            >
              Edit goal
            </button>
          </div>
...
function StatCard({ label, value, accent }: { label: string; value: string; accent: string }) {
  return (
    <div className="bg-[#0E1628] border border-white/[0.08] rounded-2xl p-4">
