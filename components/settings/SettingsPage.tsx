...
function Toggle({ checked, onChange }: { checked: boolean; onChange: () => void }) {
  return (
    <button
      onClick={onChange}
      className={`relative w-11 h-6 rounded-full transition-colors flex-shrink-0 ${checked ? 'bg-blue-600' : 'bg-white/10'}`}
    >
...
export default function SettingsPage({ profile, email }: SettingsProps) {
  ...
  return (
    <div className="p-5 lg:p-7 max-w-2xl mx-auto space-y-5">
      ...
      <section className="bg-[#0E1628] border border-amber-500/20 rounded-2xl overflow-hidden">
        ...
        <div className="px-5 py-4 space-y-5">
          ...
          {onBreak && (
            <div className="p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/25 text-sm text-amber-300">
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                <input
                  ...
                  className="bg-white/[0.04] border border-white/[0.07] rounded-xl pl-7 pr-4 py-2.5 text-sm text-white w-36 focus:outline-none focus:border-blue-500/40 transition-colors placeholder:text-slate-600"
                />
              </div>
              <button
                onClick={() => save('loss_limit', lossLimit ? parseFloat(lossLimit) : null)}
                disabled={saving === 'loss_limit'}
                className="px-4 py-2.5 bg-white/[0.04] hover:bg-white/[0.07] border border-white/[0.07] rounded-xl text-sm text-white font-semibold transition-colors disabled:opacity-50"
              >
            ...
            <div className="flex gap-2 flex-wrap">
              {[
                ...
                <button
                  ...
                  className="px-4 py-2 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/25 rounded-xl text-sm text-amber-300 font-semibold transition-colors disabled:opacity-50"
                >
          ...
          <div className="pt-4 border-t border-white/[0.05]">
            ...
            <div className="flex flex-wrap gap-2">
              <a href="https://www.gamcare.org.uk" target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-2 px-3 py-2 bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.07] rounded-xl text-xs text-slate-300 transition-colors font-medium">
