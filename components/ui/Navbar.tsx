    ...
    <nav className="border-b border-white/5 bg-[#0B0B14]/90 backdrop-blur-xl sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 bg-white/5 px-3 py-1.5">
              <div className="w-2 h-2 bg-emerald-400 animate-pulse"></div>
              <span className="text-slate-300 text-sm">{email}</span>
            </div>
            <button onClick={signOut} disabled={loading}
              className="flex items-center gap-2 text-sm text-slate-400 hover:text-white transition-all px-3 py-1.5 hover:bg-white/5 border border-transparent hover:border-white/10">
